...
class PyBulletRobotDescription(object):
    ...
    def _get_urdf_path(name):
        ...
        if not urdf_path:
            rospy.logerr(
                "[PyBulletRobotDescription::get_urdf_path] Could not get mandatory" +
                " parameter '{}urdf_path' from ROS param server, exiting now.".format(namespace))
            return None
        if not os.path.isfile(urdf_path):
            rospy.logerr(
                "[PyBulletRobotDescription::get_urdf_path] parameter 'urdf_path' is set" +
        ...
        if urdf_path[-5:] == ".urdf":
            ...
            rospy.logerr(
                "[PyBulletRobotDescription::get_urdf_path] Unknown file format of file {}, " +
